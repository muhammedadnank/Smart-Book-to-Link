# PageStream/bot/plugins/stream.py
...
from PageStream.utils.canonical_files import get_or_create_canonical_file
from PageStream.utils.database import db
from PageStream.utils.decorators import (check_banned, get_shortener_status,
                                      require_token)
from PageStream.utils.force_channel import force_channel_check
from PageStream.utils.logger import logger
from PageStream.utils.messages import (
...
from PageStream.utils.rate_limiter import handle_rate_limited_request
from PageStream.utils.file_properties import get_fname, get_file_category
    ...
    try:
        ...
        if not category:
            error_text = (
                ...
                "• 📚 **eBooks**: .pdf, .epub, .mobi, .azw, .azw3, .djvu, .fb2, .lit, .cbr, .cbz\n"
                "• 📄 **Documents**: .doc, .docx, .txt, .rtf, .odt\n"
                "• 🎧 **Audiobooks**: .mp3, .m4b, .m4a, .ogg, .flac, .aac, .wav, .opus\n"
